 ...
 * Interfaces
 *
 * Interfaces in Kotlin are very similar to Java 8. They can contain declarations of abstract
 * methods, as well as method implementations. What makes them different from abstract classes
 * is that interfaces cannot store state. They can have properties but these need to be abstract
...
/**
 * Interfaces A and B both declare functions foo() and bar(). Both of them implement foo(), but only
 * B implements bar() (bar() is not marked abstract in A, because this is the default for
 * interfaces, if the function has no body). Now, if we derive a concrete class C from A, we,
 * obviously, have to override bar() and provide an implementation.
 *
 * However, if we derive D from A and B, we need to implement all the methods which we have
 * inherited from multiple interfaces, and to specify how exactly D should implement them. This rule
